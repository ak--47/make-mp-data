 ...
 * @param {GeneratorConfig} config Optional configuration
 * @returns {string[]}
 */
export function generateSearchQueries(n = 10, config = {}) {
	const gen = buildGenerator({ ...config, style: 'search', min: 10, max: 100 });
	return gen.generateMany({ n });
}

/**
 * Generate a conversation thread (multiple related messages)
 * @param {number} length Number of messages
 * @param {GeneratorConfig} config Configuration
 * @returns {TextItem[]}
 */
export function generateThread(length = 3, config = {}) {
