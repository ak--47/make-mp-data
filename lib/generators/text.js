...
class RealisticTextGenerator {
    ...
    meetsConstraints(text, tone) {
        ...
        // Sentiment check (with tolerance)
        const score = sentiment.analyze(text).score;
        const tolerance = this.config.mixedSentiment ? 3 : 1.5;
        
        if (tone === 'pos' && score < -tolerance) return false;
        if (tone === 'neg' && score > tolerance) return false;
        if (tone === 'neu' && Math.abs(score) > tolerance * 2) return false;
     ...
     */
    isDuplicate(text) {
        const hash = simhash64(text, 4);
        
        for (const existingHash of this.generatedHashes) {
            if (hamming64(hash, existingHash) <= 3) {
                return true;
            }
        }
    ...
    /**
     * Create text object with metadata
     * @private
     */
