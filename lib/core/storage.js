	...
	async function transformThenPush(item, meta) {
		...
		if (isPassthroughHook) {
			...
			const isValidEvent = (e) => {
				if (type !== "event") return true;  // Only validate events
				if (!e || typeof e !== 'object') return false;
				// Events must have: event name, time, and a user identifier
				return e.event && e.time && (e.distinct_id || e.user_id || e.device_id);
			};
