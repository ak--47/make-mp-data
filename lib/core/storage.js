 ...
 * @param {Object} opts.context - Context object with config, batchSize, etc.
...
export class StorageManager {
     ...
     * @returns {Object} Storage containers object
     */
    async initializeContainers() {
        ...
        const storage = {
            ...
            scdTableData: [],
            groupProfilesData: await createHookArray([], {
                hook: config.hook,
                type: "group",
                filepath: `${config.simulationName || 'groups'}-GROUPS`,
                format: config.format || "csv",
                concurrency: config.concurrency || 1,
                context: this.context
            }),
            
            lookupTableData: await createHookArray([], {
                hook: config.hook,
                type: "lookup",
                filepath: `${config.simulationName || 'lookup'}-LOOKUP`,
                format: config.format || "csv",
                concurrency: config.concurrency || 1,
                context: this.context
            }),
